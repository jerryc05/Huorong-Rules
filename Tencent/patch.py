...
with open(Path(__file__).parent / 'tencent-auto.json', 'rb+') as f:
    ...
    for p in [R'*\TPDownloadProxy', R'*\TPDownloadProxy\*', R'*\Intel\ShaderCache\*', R'*\AppData\Local\D3DSCache\*', R'*\qq_guild\*',  R'*\Documents\WXWork\*', R'*\WXDrive\*', R'*\wxworkweb\*', R'*\ProgramData\boost_interprocess\*'] + [Rf'*\{x}Cache\*' for x in ('DX', 'GL')]:
        tencent.append(
            {
                'res_path': p,
                'montype': 1,
    ...
    for p in (R'*\NormalColor\shellstyle.dll', R'*\chrome.exe', R'*\DCIM\*', f"{Path.home()/'Desktop'}\\*"):
        tencent.append(
            {
                'res_path': p,
                'montype': 1,
